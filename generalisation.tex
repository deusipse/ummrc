...
We can also plot the average number of particles as the number of shots increases, whilst varying $p$. Obviously, when $p = 0$, the line is simply linear. However, things get more interesting as $p$ gets very close to 1. A plot is shown in Figure~\ref{coolgraph}, in which the average number of particles can be seen oscillating when $p = 1$. This oscillation is caused by the constant emission of X-particles, which causes it to collase every $m-n$ shots. In addition, notice that $0 \le p \le 0.5$ produce very similar values.
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
